...
Dev Server (web UI + Tailwind watch in one command, from **packrat repo root**):

```
just serve-web
```

Or run `dx serve` from `packages/web` only if you manage CSS separately (`PACKRAT_WEB_NO_TAILWIND=1 just serve-web` still runs a one-shot `npm run build` so `asset!` can find the stylesheet).

**`packages/ui/assets/tailwind.css` is generated and gitignored.** Use **`just build-ui-css`** before raw `cargo build` / `dx serve` if you are not using **`just serve-web`** (which runs Tailwind for you). **`just test-ui`** runs **`build-ui-css`** first so CI-style tests compile.

Tailwind (RSX → CSS):

- From **repo root**: `just build-ui-css` (runs `npm install` in `packages/ui` if needed, then builds CSS).
- From **`crates/packrat_ui/packages/ui`**: `npm run build` (one-shot) or `npm run watch` / `just tailwind` while editing styles.

## Generated by dioxus template from here on out
